...
const _guidanceId = process.env.VITEST_WORKER_ID
  ...
  : String(process.ppid);
const _guidanceDir = resolve(tmpdir(), `a8-cortex-guidance-${_guidanceId}`);
...
export function routePreToolUse(toolName, toolInput, projectDir) {
  ...
  if (canonical === "Bash") {
    ...
    if (/(^|\s|&&|\||\;)(curl|wget)\s/i.test(stripped)) {
      return {
        action: "modify",
        updatedInput: {
          command: 'echo "A8-Cortex: curl/wget blocked. You MUST use mcp__plugin_a8-cortex_a8-cortex__cm_fetch_and_index(url, source) to fetch URLs, or mcp__plugin_a8-cortex_a8-cortex__cm_execute(language, code) to run HTTP calls in sandbox. Do NOT retry with curl/wget."',
        },
      };
    }
    ...
    if (
      ...
      return {
        action: "modify",
        updatedInput: {
          command: 'echo "A8-Cortex: Inline HTTP blocked. Use mcp__plugin_a8-cortex_a8-cortex__cm_execute(language, code) to run HTTP calls in sandbox, or mcp__plugin_a8-cortex_a8-cortex__cm_fetch_and_index(url, source) for web pages. Do NOT retry with Bash."',
        },
      };
    }
    ...
    if (/(^|\s|&&|\||\;)(\.\/gradlew|gradlew|gradle|\.\/mvnw|mvnw|mvn)\s/i.test(stripped)) {
      ...
      return {
        action: "modify",
        updatedInput: {
          command: `echo "A8-Cortex: Build tool redirected to sandbox. Use mcp__plugin_a8-cortex_a8-cortex__cm_execute(language: \\"shell\\", code: \\"${safeCmd}\\") to run this command. Do NOT retry with Bash."`,
        },
      };
    }

    // allow all other Bash commands (passthrough)
    return null;
  }

  // ─── Read: full passthrough ───
  if (canonical === "Read") {
    return null;
  }

  // ─── Grep: full passthrough ───
  if (canonical === "Grep") {
    return null;
  }
  ...
  if (canonical === "WebFetch") {
    ...
    return {
      action: "deny",
      reason: `A8-Cortex: WebFetch blocked. Use mcp__plugin_a8-cortex_a8-cortex__cm_fetch_and_index(url: "${url}", source: "...") to fetch this URL in sandbox. Then use mcp__plugin_a8-cortex_a8-cortex__cm_search(queries: [...]) to query results. Do NOT use curl, wget, mcp_web_fetch, or mcp_fetch_tool.`,
    };
  }
  ...
  if (
    (toolName.includes("a8-cortex") && /__(cm_)?execute$/.test(toolName)) ||
    /^MCP:(cm_)?execute$/.test(toolName)
  ) {
  ...
  // Cursor can also surface the tool as MCP:ctx_execute_file.
  if (
    (toolName.includes("a8-cortex") && /__(cm_)?execute_file$/.test(toolName)) ||
    /^MCP:(cm_)?execute_file$/.test(toolName)
  ) {
    if (security) {
  ...
  if (toolName.includes("a8-cortex") && /__(cm_)?batch_execute$/.test(toolName)) {
    if (security) {
