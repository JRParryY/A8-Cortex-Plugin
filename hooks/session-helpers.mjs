/**
 * Shared session helpers for a8-cortex hooks.
 ...
 * Path: ~/<configDir>/a8-cortex/sessions/<SHA256(projectDir)[:16]>.db
 */
export function getSessionDBPath(opts = CLAUDE_OPTS) {
  const projectDir = getProjectDir(opts);
  const hash = createHash("sha256").update(projectDir).digest("hex").slice(0, 16);
  const dir = join(homedir(), opts.configDir, "a8-cortex", "sessions");
 ...
 * Path: ~/<configDir>/a8-cortex/sessions/<SHA256(projectDir)[:16]>-events.md
 */
 ...
 * Path: ~/<configDir>/a8-cortex/sessions/<SHA256(projectDir)[:16]>.cleanup
 */
