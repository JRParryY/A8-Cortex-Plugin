...
// ── Service detection for monorepo awareness ──────────────────────────────
const SERVICE_MAP = [
    [/active-8-backend\//i, "Backend"],
    [/nora\//i, "Nora"],
    [/Scheduler\//i, "Scheduler"],
    [/Active-8-frontend\//i, "Frontend"],
    [/Xtra Proj\/a8website\//i, "Website"],
    [/MD DOCS\//i, "Docs"],
    [/ADE\//i, "ADE"],
];
...
function extractFileAndRule(input) {
    ...
    if (tool_name === "Read") {
        ...
        if (service) {
            events.push({
                type: "service_touch",
                category: "service",
                data: truncate(`${service}: ${filePath}`),
                priority: 2,
            });
        }
        return events;
    ...
    if (tool_name === "Glob") {
        ...
        const globPath = String(tool_input["path"] ?? "");
        const service = detectService(globPath);
        if (service) {
            events.push({
                type: "service_touch",
                category: "service",
                data: truncate(`${service}: ${globPath}`),
                priority: 2,
            });
        }
        return events;
    ...
    if (tool_name === "Grep") {
        ...
        const service = detectService(searchPath);
        if (service) {
            events.push({
                type: "service_touch",
                category: "service",
                data: truncate(`${service}: ${searchPath}`),
                priority: 2,
            });
        }
        return events;
...
export function extractEvents(input) {
    try {
        ...
        return events.map(e => ({ ...e, data: scrubSecrets(e.data) }));
    }
    catch {
