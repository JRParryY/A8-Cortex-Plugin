...
const P1_CATEGORIES = new Set(["file", "task", "rule", "service"]);
...
/**
 * Render <active_services> from service_touch events.
 * Groups files by service, shows per-service file counts.
 */
export function renderActiveServices(serviceEvents) {
    if (serviceEvents.length === 0)
        return "";
    const serviceCounts = new Map();
    for (const ev of serviceEvents) {
        const service = ev.data.split(":")[0].trim();
        serviceCounts.set(service, (serviceCounts.get(service) ?? 0) + 1);
    }
    const lines = ["  <active_services>"];
    for (const [service, count] of serviceCounts) {
        lines.push(`    <service name="${escapeXML(service)}" files="${count}" />`);
    }
    lines.push("  </active_services>");
    return lines.join("\n");
}
/**
...
export function buildResumeSnapshot(events, opts) {
    ...
    const activeServices = renderActiveServices(serviceEvents);
    if (activeServices)
        p1Sections.push(activeServices);
