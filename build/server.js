...
process.on("unhandledRejection", (err) => {
    process.stderr.write(`[a8-cortex] unhandledRejection: ${err}\n`);
});
process.on("uncaughtException", (err) => {
    process.stderr.write(`[a8-cortex] uncaughtException: ${err?.message ?? err}\n`);
});
 ...
 * Scans ~/.claude/a8-cortex/sessions/ for *-events.md files.
...
function maybeIndexSessionEvents(store) {
    try {
        const sessionsDir = join(homedir(), ".claude", "a8-cortex", "sessions");
...
// Tool: execute
// ─────────────────────────────────────────────────────────
server.registerTool("cm_execute", {
    title: "Execute Code",
...
// ─────────────────────────────────────────────────────────
const INTENT_SEARCH_THRESHOLD = 50_000; // bytes — ~800-1000 lines
...
// Tool: execute_file
// ─────────────────────────────────────────────────────────
server.registerTool("cm_execute_file", {
...
server.registerTool("cm_search", {
    ...
    try {
        ...
        if (queryList.length === 0) {
            return trackResponse("cm_search", {
...
server.registerTool("cm_fetch_and_index", {
    ...
    try {
        ...
        if (result.exitCode !== 0) {
            return trackResponse("cm_fetch_and_index", {
                content: [
                    {
                        type: "text",
...
// Tool: batch_execute
// ─────────────────────────────────────────────────────────
server.registerTool("cm_batch_execute", {
...
server.registerTool("cm_stats", {
    ...
    // ── Header ──
    const lines = [
        `## A8-Cortex Session Report (${uptimeMin} min)`,
    ];
    ...
    try {
        ...
        const sessionDbPath = join(homedir(), ".claude", "a8-cortex", "sessions", `${dbHash}.db`);
    ...
    return trackResponse("cm_stats", {
        content: [{ type: "text", text }],
    });
});
// ── cm-doctor: diagnostics meta-tool ──────────────────────────────────────
server.registerTool("cm_doctor", {
    title: "Run Diagnostics",
    description: "Diagnose A8-Cortex installation. Returns a shell command to execute. " +
    ...
    return trackResponse("cm_doctor", {
        content: [{ type: "text", text }],
    });
});
// ── cm-upgrade: upgrade meta-tool ─────────────────────────────────────────
server.registerTool("cm_upgrade", {
    title: "Upgrade Plugin",
    description: "Upgrade A8-Cortex to the latest version. Returns a shell command to execute. " +
        "You MUST run the returned command using your shell tool (Bash, shell_execute, " +
    ...
    const text = [
        "A8-Cortex is a local installation. To update:",
        "  cd /Users/royparry/Projects/Working Project/Xtra Proj/private-context-bridge",
        "  npm run build",
        "Then restart your Claude Code session.",
        "",
        "Or use the cm-upgrade skill: /a8-cortex:cm-upgrade",
    ].join("\n");
    return trackResponse("cm_upgrade", {
        content: [{ type: "text", text }],
    });
});
// ── cm_index_project: Index architecture docs from the monorepo ──
server.registerTool("cm_index_project", {
    title: "Index Project Docs",
    description: "Index architecture docs and CLAUDE.md files from the Active-8 monorepo into the FTS5 knowledge base. Call once per session for cross-service search.",
    inputSchema: z.object({}),
}, async () => {
    const store = getStore();
    const projectDir = process.env.CLAUDE_PROJECT_DIR || process.cwd();
    ...
    const failed = [];
    const docsToIndex = [
        { path: join(projectDir, "CLAUDE.md"), source: "project:CLAUDE" },
    ...
    // Glob MD DOCS/**/*.md
    try {
        const mdDocsDir = join(projectDir, "MD DOCS");
        if (existsSync(mdDocsDir)) {
            const walkDir = (dir, prefix) => {
                for (const entry of readdirSync(dir, { withFileTypes: true })) {
                    ...
                    if (entry.isDirectory()) {
                        ...
                        docsToIndex.push({ path: fullPath, source: `docs:${prefix}/${entry.name}` });
                    }
                }
            };
            walkDir(mdDocsDir, "MD DOCS");
        }
    }
    catch { /* best effort */ }
    for (const doc of docsToIndex) {
        try {
            if (!existsSync(doc.path))
                continue;
            store.index({ path: doc.path, source: doc.source });
            indexed.push(`${doc.source} (${doc.path.split("/").pop()})`);
        }
        catch {
            failed.push(doc.source);
        }
    }
    const summary = `Indexed ${indexed.length} docs into FTS5 knowledge base.\n` +
        indexed.map(i => `  + ${i}`).join("\n") +
        (failed.length > 0 ? `\nFailed: ${failed.join(", ")}` : "") +
...
async function main() {
    ...
    console.error(`A8-Cortex MCP server v${VERSION} running on stdio`);
